...
class ValdiViewNode(nativeHandle: Long,
    ...
    override fun setStoredObject(key: InternedString, value: Any?) {
        val nativeKey = (key as? InternedStringCPP)?.nativeHandle ?: return
        NativeBridge.setStoredObjectForViewNode(nativeHandle, nativeKey, value)
    }

    override fun getStoredObject(key: InternedString): Any? {
        val nativeKey = (key as? InternedStringCPP)?.nativeHandle ?: return null
        return NativeBridge.getStoredObjectForViewNode(nativeHandle, nativeKey)
    }
