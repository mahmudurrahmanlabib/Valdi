...
bool TextLayoutBuilder::computeLineMetrics(const TextLayoutSpecs& specs,
    ...
    if (specs.replacementSize) {
        auto replacementLineMetrics = resolveReplacementLineMetrics(
            lineMetrics, specs.replacementSize.value(), specs.replacementVerticalAlignment);
...
Rect TextLayoutBuilder::computeChunkBounds(const TextLayoutSpecs& specs,
    ...
    if (specs.replacementSize) {
        lineMetrics = resolveReplacementLineMetrics(
            lineMetrics, specs.replacementSize.value(), specs.replacementVerticalAlignment);
    }
...
void TextLayoutBuilder::processUnidirectionalRun(const TextLayoutSpecs& specs,
    ...
    if (specs.replacementSize) {
        ...
        for (auto glyphIndex = glyphStartIndex; glyphIndex < glyphEndIndex; glyphIndex++) {
            _glyphs[glyphIndex].advanceX = glyphIndex == glyphStartIndex ? specs.replacementSize->width : 0.0f;
        }
    }
...
static void appendSegmentWord(const TextLayoutBuilderSegment& segment,
    ...
    // TODO: backgroundStyle->horizontalPadding() is dropped here, so justified text loses its
    // background padding entirely. The fix requires knowing whether this is the first/last word
    // of the segment (to add padding.left / padding.right respectively) and creating per-word
    // background style entries with the correct padding, similar to how buildSegments() zeroes
    // inner padding for multi-font runs.
...
void TextLayoutBuilder::appendBackgroundIfNeeded(std::vector<TextLayoutVisualEntry>& visualEntries,
    ...
    visualEntries.emplace_back(
        Rect::makeXYWH(resolvedSegmentX, segment.drawPosition.y, segment.bounds.width(), segment.bounds.height()),
        true,
...
// TODO: segment.bounds.width() includes backgroundStyle->horizontalPadding(), which causes two
// structural bugs: (1) decorations (underline/strikethrough) span the padding rather than just
// the text glyphs, and (2) multi-line wrapped segments get inconsistent left-padding indentation
// because buildSegments() zeroes inner padding before line-breaking. The fix requires managing
// background padding during line/background assembly (e.g. calculateLineOffsets or buildVisualEntries)
// rather than baking it into segment.bounds before line breaking.
void TextLayoutBuilder::appendDecorationIfNeeded(std::vector<TextLayoutVisualEntry>& visualEntries,
    ...
    switch (segment.textDecoration) {
        ...
        case TextDecorationDottedUnderline: {
            ...
            auto underlinePosition =
                segment.textDecoration == TextDecorationDottedUnderline ?
            ...
            if (customUnderlineStyle) {
                ...
                underlinePosition =
                    (metrics.descent / 2.0f) + customUnderlineStyle->offset - (underlineThickness / 2.0f);
            }

            auto& visualEntry = visualEntries.emplace_back();
            visualEntry.bounds = Rect::makeXYWH(
                resolvedSegmentX, resolvedSegmentY + underlinePosition, segment.bounds.width(), underlineThickness);
    ...
    for (size_t colorIndex = 0; colorIndex < _colors.size(); colorIndex++) {
        ...
        for (const auto& segment : _segments) {
            ...
            if (segment.attachment != nullptr) {
                auto attachmentBounds = segment.isReplacement ? Rect::makeXYWH(resolvedSegmentX,
