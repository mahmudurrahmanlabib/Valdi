...
static void computeFlexBasisForChild(
  ...
  if (useResolvedFlexBasis) {
    ...
    // non-measure container children ONLY when the parent is a column
    // scroll view. This prevents unnecessary re-measurement cascades when
    // a sibling changes size in a vertical ScrollView, while still
    // propagating height constraints through container children in
    // non-scroll contexts (needed for percentage height resolution in
    // grandchildren).
    //
    ...
    if (fixFlexBasisFitContent &&
        ...
        child->style().overflow() != Overflow::Scroll) {
    ...
    if (sizingModeMainDim != SizingMode::StretchFit) {
      ...
      if (yoga::isDefined(minInnerMainDim) &&
        ...
        if (!useLegacyStretchBehaviour &&
          ...
          // Use min() so we shrink to content but never grow beyond
          // available space. In Yoga 1.x this block was dead code
          // (YGFloatIsUndefined(X) && X == 0 is always false). Yoga 2.x
          // fixed the condition, but unconditional assignment broke
          // layouts where content exceeds the container (e.g. scroll
          // containers and rows with >100% percentage children).
          availableInnerMainDim =
              std::min(availableInnerMainDim, flexLine.sizeConsumed);
        }
